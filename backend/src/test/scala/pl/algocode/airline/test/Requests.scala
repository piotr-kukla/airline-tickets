package pl.algocode.airline.test

import pl.algocode.airline.Fail
import pl.algocode.airline.passwordreset.PasswordResetApi
import pl.algocode.airline.passwordreset.PasswordResetApi.*
import pl.algocode.airline.user.UserApi
import pl.algocode.airline.user.UserApi.*
import pl.algocode.airline.util.Strings.asId
import sttp.client4.*
...
class Requests(backend: SyncBackend):
  ...
  def randomLoginEmailPassword(): (String, String, String) =
    (random.nextString(12), s"user${random.nextInt(9000)}@airline.com", random.nextString(12))
