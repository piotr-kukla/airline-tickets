...
import pl.algocode.airline.email.sender.DummyEmailSender
import pl.algocode.airline.test.*
...
class PasswordResetApiTest extends BaseTest with Eventually with TestDependencies:
  ...
  def codeSentToEmail(email: String): String =
    ...
    val emailData = DummyEmailSender
      .findSentEmail(email, "SoftwareMill Airline password reset")
  ...
  def codeWasNotSentToEmail(email: String): Unit =
    ...
    val maybeEmail = DummyEmailSender.findSentEmail(email, "SoftwareMill Airline password reset")
    maybeEmail match
      case Some(emailData) =>
