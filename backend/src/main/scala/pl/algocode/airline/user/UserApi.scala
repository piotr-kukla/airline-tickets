...
import pl.algocode.airline.http.{EndpointsForDocs, ServerEndpoints}
import pl.algocode.airline.infrastructure.DB
import pl.algocode.airline.metrics.Metrics
import pl.algocode.airline.security.{ApiKey, Auth}
import pl.algocode.airline.util.Strings.{Id, asId}
...
object UserApi extends EndpointsForDocs:
  ...
  val registerUserEndpoint = baseEndpoint.post
    ...
    .in(jsonBody[Register_IN].example(Register_IN("john", "john@airline.com", "1234_abcd_ABCD")))
